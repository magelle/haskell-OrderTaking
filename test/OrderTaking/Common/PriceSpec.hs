...
spec = describe "Price" $ do
    describe "Create a Price" $ do
        it "should accept to create unit quantity" $
            isOk (create 50.0) `shouldBe` True
        ...
        it "should refuse to create a unit quantity under limits" $
            create (-0.1) `shouldBe` Error "Price: Must not be less than 0.0"
        it "should refuse to create a unit quantity under limits" $
            create 1000.1 `shouldBe` Error "Price: Must not be greater than 1000.0"
