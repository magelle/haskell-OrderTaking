...
spec = describe "BillingAmount" $ do
    describe "Create a billing amount" $ do
        it "should accept to create billing amount" $
            isOk (BillingAmount.create 50.0) `shouldBe` True
        ...
        it "should refuse to create a billing amount under limits" $
            BillingAmount.create (-0.1) `shouldBe` Error "BillingAmount: Must not be less than 0.0"
        it "should refuse to create a billing amount under limits" $
            BillingAmount.create 10000.1 `shouldBe` Error "BillingAmount: Must not be greater than 10000.0"
...
valueR (Ok a) = BillingAmount.value a
unwrapR (Ok a) = a
